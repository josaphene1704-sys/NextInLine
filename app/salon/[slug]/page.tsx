    ...
    <main className="min-h-screen">
      <header className="sticky top-0 z-20 glass-header">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            {business?.logoUrl ? (
              <Image src={business.logoUrl} alt="לוגו" width={28} height={28} className="rounded-md object-cover shrink-0" />
            ) : (
        ...
        <div className="pb-6 text-center">
          {business.imageUrl && (
            <div className="relative w-full aspect-[3/1] overflow-hidden">
              <Image src={business.imageUrl} alt={t(business.name)} fill className="object-cover" sizes="100vw" priority />
          ...
          <div className={business.imageUrl ? "-mt-10 relative z-10 px-4" : "bg-gradient-to-b from-primary/8 via-primary/4 to-transparent pt-10 px-4"}>
            {business.logoUrl && (
              <Image src={business.logoUrl} alt="לוגו" width={72} height={72} className="rounded-2xl object-cover mx-auto mb-3 shadow-md border-2 border-card" />
            )}
            ...
            <div className="flex items-center justify-center gap-4 text-xs text-muted-foreground flex-wrap">
              ...
              {business.phone && (
                <span className="flex items-center gap-1">
                  ...
                  <a href={`tel:${business.phone}`} className="hover:text-foreground transition-colors">{business.phone}</a>
                </span>
              )}
            </div>
      ...
      {business === null && (
        <div className="text-center py-24 px-4 text-muted-foreground">
          ...
          <p className="text-sm">הסלון לא נמצא במערכת</p>
        </div>
      )}
