...
// ─── Profile view ─────────────────────────────────────────────────────────────
//
// Shared between the global profile (/profile) and the per-salon profile
// (/salon/[slug]/profile). When `filterBusinessId` is provided, only that
// salon's appointments and waiting-list entries are shown; `backHref` controls
// where the back arrow and empty-state CTA point.

export function ProfileView({
  filterBusinessId,
  backHref = "/",
}: {
  filterBusinessId?: Id<"businesses">;
  backHref?: string;
}) {
  const { user } = useAuth();
  const router   = useRouter();

  const appointmentsRaw = useQuery(
    api.appointments.getCustomerProfile,
    user ? { customerPhone: user.phone } : "skip"
  );
  const waitingRaw = useQuery(
    api.waitingList.getForCustomer,
    user ? { customerPhone: user.phone } : "skip"
  );

  if (user === null) { router.replace(backHref); return null; }
  if (!user) return null;

  // Scope to a single salon when requested.
  const appointments = filterBusinessId
    ? appointmentsRaw?.filter(a => a.businessId === filterBusinessId)
    : appointmentsRaw;
  const waitingEntries = filterBusinessId
    ? waitingRaw?.filter(e => e.businessId === filterBusinessId)
    : waitingRaw;
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        {appointments?.length === 0 && activeWaiting.length === 0 && (
          <div className="text-center py-16 text-muted-foreground">
            ...
            <button onClick={() => router.push(backHref)} className="mt-4 text-sm text-primary hover:underline">
              הזמני תור עכשיו →
            </button>
          </div>
        )}
