  ...
  return (
    <>
      <li className="space-y-1.5">
        {/* Single info row: date · time · separator · service */}
        <div className="flex items-center gap-1.5 text-xs flex-wrap">
          <span className="font-medium text-foreground">
            {fmtDate(appt.startTime, lang)}
          </span>
          <span className="text-muted-foreground">
            {formatSlotTime(appt.startTime, timezone, lang)}
          </span>
          {serviceName && (
            <>
        ...
        {/* Action buttons — compact, inline, auto-width */}
        {!confirmCancel ? (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {cancelError && (
              <span className="text-[11px] text-destructive">{cancelError}</span>
            )}
  ...
  return (
    <div className="glass rounded-2xl px-4 py-3 mb-6 border border-amber-400/30 bg-amber-50/40 dark:bg-amber-900/10 space-y-3">
      ...
      <ul className="space-y-3 divide-y divide-amber-200/40 dark:divide-amber-700/20">
        {upcoming.slice(0, 3).map((appt, i) => (
          <div key={appt._id} className={i > 0 ? "pt-3" : ""}>
            <ApptRow appt={appt} timezone={timezone} lang={lang} />
          </div>
        ))}
      </ul>
    </div>
